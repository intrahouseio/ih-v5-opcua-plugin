...
class ConnectionManager {
    constructor(plugin) {
        ...
        this.connectionState = 'disconnected';  // Начальное состояние
        // History соединение
    ...
    async getHistorySession(params) {
        if (this._isHistorySessionAlive()) {
            this.plugin.log(`[History] Reusing existing session`, 2);
            return this.historySession;
        }

        this.plugin.log(`[History] Creating new history session...`, 1);
        await this._createHistorySession(params);
        return this.historySession;
    }

    _isHistorySessionAlive() {
        return this.historySession && 
               !this.historySession.isClosed && 
               this.historySession.sessionId;
    }

    async _createHistorySession(params) {
        await this._closeHistorySessionInternal();

        const targetEndpoint = params.endpointUrl || params.primary_endpointUrl;

        const { clientCM, privateKeyFile, certificateDerFile, certificateFile } = 
            await certmanager.start(this.plugin, this.plugin.opt.pluginbasepath + "/" + this.plugin.opt.id);

        this.historyClient = OPCUAClient.create({
            applicationName: "IntraHistoryClient",
            connectionStrategy: { initialDelay: 1000, maxDelay: 5000, maxRetry: 3 },
            securityMode: MessageSecurityMode[params.messageSecurityMode],
            securityPolicy: SecurityPolicy[params.securityPolicy],
            endpointMustExist: false,
            certificateFile,
            privateKeyFile,
            clientCertificateManager: clientCM,
            ...
            requestedSessionTimeout: 600000, // 10 минут
        });

        await this.historyClient.connect(targetEndpoint);

        let certificateData, privateKeyObject;
        if (params.auth_type === 'Certificate') {
            if (params.autogen_cert) {
                certificateData = await fs.readFile(certificateDerFile);
                const pem = await fs.readFile(privateKeyFile, "utf8");
                privateKeyObject = createPrivateKey({ key: pem, format: "pem" });
            } else {
                certificateData = await fs.readFile(params.certDer);
                const pem = await fs.readFile(params.privateKey, "utf8");
                privateKeyObject = createPrivateKey({ key: pem, format: "pem" });
            }
        }

        const userIdentity = {
            type: UserTokenType[params.auth_type],
        ...
        this.historySession = await this.historyClient.createSession(userIdentity);
        
        this.plugin.log(`[History] Session created (will auto-close after ~10 min idle by server)`, 1);
    }

    async _closeHistorySessionInternal() {
        if (this.historySession) {
            try { await this.historySession.close(); } catch (e) {}
            this.historySession = null;
        }
        if (this.historyClient) {
            try { await this.historyClient.disconnect(); } catch (e) {}
            this.historyClient = null;
        }
    }
    ...
    _sendProcInfo() {
        process.send({
            ...
            data: {
                ...
                current_server: this.redundancy,
                current_endpoint: this.client ? this.client.endpointUrl : null
            }
        });
    }

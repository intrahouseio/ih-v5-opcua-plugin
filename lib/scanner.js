      ...
      for (const ref of browseResult.references) {
        ...
        if (ref.nodeClass == 1) {
          ...
          if (typeof data.dataType != "undefined") {
            ...
            //console.log(
            //  'channel: ' + data.displayName.text + ' dtype=' + dtype + ' dataValue=' + curValue + " accessLevel=" + accessLevel
            //);
            this.scanArray.push({
              nodeId: ref.nodeId,
              parentId,
              id: chan,
              title: data.displayName.text + ' (' + dtype + ') ' + curValue,
              dataType: dtype,
              accessLevel: accessLevel,
              channel: { topic: data.displayName.text, dataType: dtype, displayName: data.displayName.text, chan: chan, accessLevel: accessLevel }
            });
          }
        }
      }

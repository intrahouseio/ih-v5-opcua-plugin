...
class Scanner {
  ...
  async scanning(nodeId, referenceTypeId) {
    try {
      const parentId = getId(nodeId) + getId(referenceTypeId);
      ...
      for (const ref of browseResult.references) {
        ...
        if (ref.nodeClass == 2 && ref.nodeId.identifierType) {
          ...
          const curValue = data.dataType.value != 22 ? JSON.stringify(data.value) : data.value;
        ...
        if (ref.nodeClass == 4 && ref.nodeId.identifierType) {
          const { identifierType, identifierString } = getIdentifierTypeAndString(ref.nodeId.identifierType);
          let chan;
          if (identifierType == 'BYTEString') {
            chan = 'ns=' + ref.nodeId.namespace + identifierString + ref.nodeId.value.toString('base64');

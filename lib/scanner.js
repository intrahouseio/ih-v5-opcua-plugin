      ...
      for (let i = 0; i < browseResult.references.length; i++) {
        ...
        if (ref.nodeClass == 1) {
          ...
          if (refs.references.length > 0) {
            ...
            this.scanArray.push(branch);
            this.sendTreePart({ ...branch, children: [] }, parentId);
            const leaf = await createVariableLeaf(ref, chan, parentId, parentNodeId, this.session, this.plugin)
...
async function createVariableLeaf(ref, chan, parentId, parentNodeId, session, plugin) {

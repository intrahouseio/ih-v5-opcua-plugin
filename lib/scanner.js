...
const { AttributeIds, opcua } = require('node-opcua');
...
class Scanner {
  ...
  async scanning(nodeId, referenceTypeId) {
    try {
      let parentId = getId(nodeId) + getId(referenceTypeId);
      ...
      for (const ref of browseResult.references) {
        ...
        if (ref.nodeClass == 1) {
          ...
          this.idSet.add(id);
          const branch = { nodeId: ref.nodeId, parentId, id, title: displayName };
          ...
          if (data.dataType.value == 24 || data.dataType.value == 22 || typeof data.dataType.value == 'string'/*|| typeof data.value == 'object'*/ || data.arrayDimensions) {
           ...
           const id = getId(ref.nodeId) + getId(ref.referenceTypeId);
            ...
            const dtype = typeof data.dataType != 'undefined' ? getDtype(data.dataType.value) : '';
